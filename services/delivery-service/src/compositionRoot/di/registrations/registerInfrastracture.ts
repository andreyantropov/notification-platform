...
import { createConsumer } from "../../../infrastructure/amqp/index.js";
import {
  createBitrixChannel,
  createEmailChannel,
...
export const registerInfrastracture = (
  ...
  container.register({
    ...
    consumer: asFunction(({ broker, env, sendNotificationUseCase }) => {
      const channelPromise = broker.createChannel();
      const consumer = createConsumer(
        { channelPromise, sendNotificationUseCase },
        {
          queue: env.CONSUMER_QUEUE,
          prefetchCount: env.CONSUMER_PREFETCH_COUNT,
        },
      );
