    ...
    paths: {
      "/v1/notifications": {
        post: {
          ...
          description:
            "Принимает уведомление, валидирует его и ставит в очередь на доставку.\n" +
            "**Важно:** Успешный ответ (200) означает лишь то, что уведомление принято в работу. " +
            "Физическая отправка через каналы (Email, Bitrix) происходит асинхронно.\n" +
            "Гарантии доставки определяются выбранной стратегией (`strategy`).",
          tags: ["Notifications"],
          security: [{ BearerAuth: [] }],
          requestBody: {
            required: true,
          ...
          responses: {
            200: {
              description:
                "Уведомление прошло валидацию и успешно поставлено в очередь.",
              content: {
                "application/json": {
                  schema: {
                    $ref: "#/components/schemas/Notification",
      ...
      "/v1/notifications/batch": {
        post: {
          ...
          description:
            "Принимает пакет уведомлений (до 50 шт.) для постановки в очередь.\n" +
            "Каждое уведомление валидируется независимо. Возвращает детальный отчет: " +
            "какие элементы приняты в работу (поставлены в очередь), а какие отклонены.\n" +
            "**Важно:** Принятие в очередь не гарантирует мгновенную доставку.",
          tags: ["Notifications"],
          security: [{ BearerAuth: [] }],
          requestBody: {
            required: true,
          ...
          responses: {
            202: {
              description:
                "Все уведомления в пакете прошли валидацию и приняты в очередь на обработку.",
              content: {
                "application/json": {
                  schema: {
